##Script to generate SQLite database for COMBAT 10X-VDJ-cellranger outputs
#Author: Bo Sun
...
##Create SQLitedb
conn <- RSQLite::dbConnect(RSQLite::SQLite(), "/well/combat/projects/repertoire/imgtout/TCR/TCRdb/TCRIMGT.db")
lapply(TCR$all_contig_annotations, function(x){ RSQLite::dbWriteTable(conn,"10X_filtered_contig_annotations", x, append = TRUE)})
dbListTables(conn)
dbDisconnect(conn)

conn <- RSQLite::dbConnect(RSQLite::SQLite(), "/well/combat/projects/repertoire/imgtout/BCR/BCRdb/BCRIMGT.db")
