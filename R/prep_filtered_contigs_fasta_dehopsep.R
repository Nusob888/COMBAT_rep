...
##This script is to replicate the concatenation and conversion of dehopped all_contig.fasta repertoire files from COMBAT study to IMGT compatible fasta files. 
#Author: Bo Sun
...
saveRDS(repdir, paste0(wd$data, "repdir_dehop_all_contig.rds"))
...
for (i in 1:length(TCRsplit)){
  filename <- paste(names(TCRsplit), "fasta", sep = "_COMBAT_TCR_all_contig_dehopsep.")
  writeXStringSet(TCRsplit[[i]], paste0(wd$output, filename[i]), append=FALSE,
                  compress=FALSE, compression_level=NA, format="fasta")
}

for (i in 1:length(BCRsplit)){
  filename <- paste(names(BCRsplit), "fasta", sep = "_COMBAT_BCR_all_contig_dehopsep.")
